...
#
# PORT is 5174 rather than Next's default 3000, matching `npm run dev` and the
# compose mapping so the app answers on the same port everywhere. It is still read
# from the environment, so `-e PORT=…` overrides it.
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||5174)+'/login').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
